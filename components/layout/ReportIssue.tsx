...
import { Bug, Clock3, FileDown, Send, X } from "lucide-react";
...
function MetaChip({
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-2 rounded-sm border border-border-muted bg-surface-muted/80 px-2.5 py-2 sm:gap-2.5 sm:px-3 sm:py-2.5">
      <span className="inline-flex size-7 shrink-0 items-center justify-center rounded-xs bg-surface-raised text-accent-amber sm:size-8">
        {icon}
      </span>
      ...
      {open ? (
        ...
        >
          <button
            ...
            className="absolute inset-0 bg-surface-base/75 backdrop-blur-sm"
            onClick={onClose}
          />
          ...
          <motion.div
            ...
            className={cn(
              ...
              "border border-border-muted bg-surface-raised shadow-accent-lg",
              /* Mobile: bottom sheet within dynamic viewport; desktop: centered card */
              "max-h-[min(92dvh,920px)] rounded-t-md sm:max-h-[min(85vh,720px)] sm:rounded-md",
          ...
          >
            {/* Header — fixed */}
            <div className="flex shrink-0 items-start justify-between gap-3 border-b border-border-muted px-4 py-3.5 sm:px-6 sm:py-4">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="inline-flex size-8 shrink-0 items-center justify-center rounded-full bg-rose-500/15 text-rose-400">
                    <Bug size={16} aria-hidden />
                  </span>
                  <h2
                    id={titleId}
                    className="font-mono text-[13px] font-bold tracking-[0.1em] text-rose-400 uppercase sm:text-[15px] sm:tracking-[0.12em]"
                  >
                    Something not working?
                  </h2>
                </div>
                <p className="mt-1.5 text-[13px] leading-relaxed text-text-secondary sm:mt-2 sm:text-sm">
            ...
            >
              {/* Scrollable fields */}
              <div className="min-h-0 flex-1 space-y-3.5 overflow-y-auto overscroll-contain px-4 py-3.5 sm:space-y-4 sm:px-6 sm:py-5">
                ...
                <Field
                  ...
                  label="What happened?*"
                  error={
                    invalid ? "Please describe what went wrong." : undefined
                  }
                >
                  <Textarea
                    id={`${formId}-happened`}
              ...
              <div className="shrink-0 border-t border-border-muted bg-surface-raised px-4 py-3 sm:px-6 sm:py-4">
                <Button
                  type="submit"
                  size="lg"
                  fullWidth
                  magnetic={false}
                  className="bg-accent-amber text-text-inverse hover:bg-[color-mix(in_srgb,var(--theme-accent-amber)_88%,black)]"
                >
                  Send Report
